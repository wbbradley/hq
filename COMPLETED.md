...
project, agent, provider, and draft identities through the application, SQLite store, local API,
client, and TUI. Inbox renders one selectable “Conversation not started” setup rather than a fake
conversation or implicit filter. Esc retains it, `r`/Enter and repeated `n` reopen the same draft,
Tab moves between real panes, and `c` explicitly replaces the chosen available agent without
changing the draft identity.

First-message commit and restart recovery correlate the retained setup to pending input and the
authoritative root/thread by exact draft/message identity. The setup presentation disappears only
for its matching conversation while typed recovery evidence remains until the assignment is
runnable, preserving exact-once activation or handoff after response uncertainty or restart.

Pure model, protocol, store, render, reconnect, and restart tests cover the typed lifecycle,
idempotent reopening, explicit agent replacement, wide and compact layouts, and absence of hidden
filters or duplicate-looking rows. The installed bootstrap scenario now closes and reopens the
first composer, sends and activates the project conversation, observes authoritative replacement,
and continues it. Formatting, strict locked workspace Clippy, and the complete locked workspace
suite pass.
...
participant-first Inbox list and give remaining width to Conversation; compact Conversation and
Draft focus retain only the selected summary above the transcript. Messages and compact typed
activity render at the pane edge, wrapped entry heights are measured with Ratatui around the stable
fact anchor, and full-row selection no longer inserts a marker or moves text.

Added closed author, activity-status, and focused/retained transcript-selection theme roles across
terminal, no-color, Base16, native inheritance, style tests, and documentation. Older-page requests
now retain typed cursor provenance so loading and failure preserve the transcript and anchor. Exact
routing, semantics, evidence, activity state, and raw detail moved out of transcript rows into a
bounded wide inspector or compact/draft-safe secondary pane; Back closes details before leaving the
conversation. Participant hierarchy, onboarding copy, snapshots, model and mapper contracts,
installed PTY coverage, acceptance documentation, and obsolete-label assertions were updated. The
stack was rebased onto the Gruvbox-default mainline, and formatting, architecture/qualification
validation, strict workspace Clippy, and the complete locked all-target/all-feature suite pass.
...
## 2026-08-30 — Modeless Inbox composition for project conversations

Moved all ordinary message writing, including project messages, into a durable modeless composer in
the Inbox workspace. Project drafts now carry typed project and optional exact-thread targets
through the TUI, node, local API v1, application planner, and in-place schema-v1 persistence. A
project continuation is validated against and causally joins its exact authoritative root, while
New conversation creates a distinct root and selects it by committed-message identity.

Removed the project-details instruction form and its shortcuts, kept the lower-level project send
path for CLI orchestration, and made failures preserve the draft beside the selected conversation.
The guided new-project journey now skips confirmation when only one agent service is available,
activates and dispatches the committed first thread once, and lands on that exact Inbox
conversation. Updated protocol and TUI documentation and expanded reducer, persistence, local API,
model, rendering, executor, and installed PTY coverage. Formatting, strict workspace Clippy, the
complete locked all-target/all-feature test suite, architecture verification, and qualification
validation all pass.
...
snapshot/selected-page views before first draw and on every Inbox selection. A separate latest-value
observation control wakes idle Unix reads without reconnecting or blocking behind commands, while
an encapsulated typed presentation directory is shared with the explicit older-page command path.

The pure model atomically applies list and detail, suppresses stale or mismatched views, chooses
deterministic successors, retains eight revision-tagged first pages, and never renders passive
first-page loading. Startup, rapid selection, blocked commands, cache eviction, reconnect, wake
generation and partial-frame handling, shell behavior, and installed PTY regressions cover the
boundary. The Rust design, acceptance, behavior-ledger, and TUI documentation now describe the
materialized observation contract.
...
Conversation, Agent, and Folder. The approved design makes Inbox collaboration the primary project
action, defines deterministic zero/one/many-conversation behavior, replaces the modal shortcut wall
with labeled state-dependent administration, and reserves dialogs for bounded destructive or force
decisions.

Added exhaustive current-state and command classification, wide and compact wireframes, focus and
Back behavior, scenario walkthroughs, progressive-disclosure invariants, and a staged migration away
from `UiProjectModal::Details` in `docs/rust/projects-workspace.md`, with the TUI guide linked and
the current modal marked as legacy. Formatting, architecture and qualification validation, strict
workspace Clippy, and the complete locked workspace test suite pass. The review decisions were
explicitly approved; implementation is queued separately.
...
## 2026-08-30 — Chat-like Inbox conversation surface specification

Specified Inbox as a participant-led messaging surface rather than a protocol-record viewer. The
approved design bounds the conversation list so the transcript dominates, starts full-width
author/body blocks at column zero, replaces technical message chrome with explicit people and
compact typed activity, makes paging visible only when actionable, and uses theme-derived full-row
focus without shifting text.

Added wide, compact, selected-inspector, and modeless-draft wireframes; loading, paging, failure,
wrapped-height, no-color, semantic-theme, typed-presentation, migration, and acceptance contracts in
`docs/rust/inbox-conversation-surface.md`. Linked the review draft from the active TUI contract and
queued implementation separately. Formatting, architecture and qualification validation, strict
workspace Clippy, and the complete locked workspace test suite pass. The five review decisions were
explicitly approved.
...
## 2026-08-31 — Clear live and completed agent work

Added a provider-neutral, bounded completed-item presentation model for commands, file changes,
tools, and web searches, carrying it through Codex normalization, canonical facts, reducers,
persistence, local APIs, node mapping, and the TUI. Command previews preserve safe multiline
structure, show exit state and up to three output lines, expose truncation, and strip terminal
escape/control sequences while retaining bounded technical detail.

Conversation queries now consolidate transient progress below the TUI into one pagination-safe live
tail selected by full operation identity. The latest useful progress replaces the running fallback,
terminal lifecycle evidence removes the live row, durable completed work stays in history, and TUI
refresh/paging preserves tail following or a reader's logical anchor. Long-history, retention,
reopen, repair, typed-family, render-height, and terminal-safety coverage passes with strict Clippy,
the complete workspace suite, and all installed PTY regressions.
...
## 2026-09-01 — Human responses to provider interactions

HQ now carries provider-neutral questions, command/file approvals, permissions, and MCP
elicitations through application ports and local API v1. Acknowledged local-API sessions own exact
responder leases; reconnects register fresh identities, refresh pending interactions immediately,
and consume operation invalidations without waiting for a durable revision change. Equal answer
retries reconcile and conflicting reuse is rejected.

The harness retains requests only while a responder can receive them and fails them closed in
source order when the last responder leaves or the exact provider operation terminates. The TUI
shows a bounded interaction queue with agent/project context, typed choices or free text, explicit
cancel/deny behavior, stale recovery, and exact technical evidence. Codex request shapes and an
installed fake-Codex approval round trip cover the complete path.
...
## 2026-09-01 — Authoritative current conversation activity

The centralized conversation-tail query now retires progress using exact operation/item completion
evidence and source sequence. New local-human input forms a freshness boundary, with a typed running
turn as fallback until later provider activity arrives. This prevents completed test output and
older telemetry from being presented below a newer human message as current work.

Pending provider interactions exclusively own live presentation for their exact operation without
discarding canonical history. Store/query regressions cover completion ordering, concurrent
operations, later activity, reopen/repair, and the reported failed-test plus “Are you still
working?” sequence; TUI rendering preserves stable selection while suppressing redundant running
status.
...
## 2026-09-03 — Bounded canonical-ingest architecture

A deterministic mixed-activity fixture now records canonical, affected-edge, database-size,
complete-reduction, repair, append, and actor-read observations at configurable sizes. Structural
tests prove that the existing set expansion emits exactly `n × (n - 1)` directed edges and that a
collision-checked typed group-membership prototype preserves overlapping closure with linear
membership rows. The measured 32-to-512 replacement series and the live Alice workload establish
that the existing primary key is not missing: dense closure and unconditional complete
reduction/staging/diff/readback are independent scaling causes.

The storage contract now audits every impact structure and production reader, specifies the strict
typed impact-group schema and affected-key fixed-point algorithm, preserves complete reduction as
the repair and differential oracle, and defines cache, ingress-coalescing, actor-fairness,
crash-consistency, rollout, deletion, and measurable acceptance boundaries. Five storage acceptance
scenarios cover compact membership, batch equality, old-plus-new retraction, crash recovery, and
provider-burst read fairness. Strict Clippy and the complete locked all-target/all-feature workspace
suite pass.
...
## 2026-09-03 — Guided project creation continuation

The global New workflow now owns a typed project-work coordinator with explicit project-picker,
creation, reconciliation, authoritative-refresh, and agent-choice states. Existing-folder and
isolated-worktree creation retain their exact parent selection and Back destinations; successful
creation selects the returned project identity and advances once to its agent chooser without
leaving stale creation surfaces underneath.

Nonterminal worktree commands retain and replay their exact encoded command through the long-lived
local client. The TUI correlates continuation results by effect, command, operation, project, and
action identity, treats running work as non-cancellable, bounds authoritative project refreshes,
and preserves exact entered values and recovery evidence without submitting duplicate Git work.

Pure reducer, reconnecting-client, executor, render, and installed real-Git PTY coverage exercises
multi-step completion, reconnect/reconciliation, explicit Back navigation, and single worktree
creation. The workflow documentation was updated, and formatting, strict locked workspace Clippy,
and the complete locked all-target/all-feature workspace suite pass.
...
## 2026-09-04 — Conversation-scoped command approvals

Command approvals now appear as request-keyed inline controls in the lower pane of their exactly
correlated conversation. Typed project/agent/provider/session evidence selects the conversation,
loaded running-operation evidence verifies it, and pending requests are remapped whenever
authoritative snapshots or pages change. Missing, ambiguous, and contradictory mappings remain
visible as nonmodal recovery evidence instead of being guessed.

The inline control has its own focus and independently retains choice, submission, and failure
state. It preserves the transcript viewport and any exact draft state behind it, allows modeless
view switching and unrelated replies, and correlates completion by effect plus request identity.
Questions, file approvals, permissions, and MCP interactions retain true modal scope.

Pure model, mapping, render, shell-wakeup, and installed fake-Codex PTY coverage proves independent
simultaneous requests, navigation away and back, draft restoration, stale completion isolation,
unresolved recovery, modal capture, and exact tail suppression. Formatting, strict locked workspace
Clippy, and the complete locked all-target/all-feature workspace suite pass.
...
## 2026-09-04 — Whole-conversation archive and agent-oriented Inbox

Conversation archive is now one canonical, absorbing lifecycle fact keyed by the exact typed
conversation identity. Archiving retains the complete immutable transcript, has no restore action,
and no longer exposes message-level archive or restore commands. In the Inbox, `d` confirms and
archives the selected conversation; agent-backed work is stopped first through its exact project or
provider-session evidence, and a failed stop leaves the conversation active.

Inbox now presents every active registered agent by stable `AgentId`, ordered by canonical
communication recency with deterministic ordering for agents without conversations. Rows retain
typed project and session context, advance to another active conversation after archive, and remain
selectable when no conversation exists. Enter on an agent without a conversation joins the same
guided project-binding workflow used by New, while direct messaging lives only under `n`.

Conversation aliases, selected views, and pending approvals reconcile atomically so input cannot
observe a half-remapped project-row-to-agent-row transition. Protocol documentation and fixtures now
cover FCT-049 explicitly and catalog tests select facts by stable family identity rather than list
position. Formatting, strict workspace Clippy, the complete workspace test suite, and all 20
installed pseudoterminal journeys pass.
...
## 2026-09-05 — Inbox full-pane typed routes

Inbox, Sent, and Archived now use typed workspace, conversation, and exact transcript-evidence
routes at every terminal width. Passive materialized pages remain bounded caches until Enter
opens their stable row identity; Escape pops one visible level; same-workspace shortcuts restore
the root; refresh removal and late page completions cannot steal navigation. Drafts and scoped
approvals remain adjacent to their owning route, while list and conversation rendering no longer
share master/detail panes. Model, render, redraw-budget, CLI, and installed PTY coverage was
updated, including deterministic synchronization on authoritative project input projection.

Strict workspace Clippy and the complete workspace test suite pass.
...
Rewrote the TUI, Inbox, and Projects guides around the implemented typed navigation stack,
destination-rooted links, full-pane route ownership, workflow completion, and conversation-scoped
adjacency. Reconciled the acceptance matrix and behavior ledger, captured typed root/detail/Back
paths at the shell boundary, and qualified the visible breadcrumb through the installed terminal.
...
## Conversation readiness and message presentation

Implemented shared typed transcript visibility with navigation, viewport, and detail cleanup while retaining successful turn evidence for automatic follow-up drafts. Running work keeps an explicit readiness indicator alongside progress. Structured commands retain highlighted command/output previews and place failure/interruption footers after output. Local messages use configurable full-width semantic bands with readable Markdown and delivery labels across terminal, no-color, Base16, and native themes. Updated installed PTY scenarios to gate completion on visible work and verify continuation readiness. Validation: formatting, strict workspace Clippy, and all 1,113 Rust workspace tests passed; focused TUI tests also passed after the final label-preservation adjustment.
...
Unix Codex launches now own isolated process groups. Forced cleanup kills the group; natural leader exit is observed without reaping until descendant cleanup completes, preserving the group identity and original exit status. A mutex-protected terminal result makes repeated waits and kills safe. Added real child/grandchild and unrelated-process regressions for forced cleanup and early leader exit. Formatting, strict workspace Clippy, hq-codex tests, and all 1,115 workspace tests passed. The additional architecture script check still fails on pre-existing stale composition checks; no changes to that script were retained.
...
## Character and word editing shortcuts

Added editing-specific Ctrl-B/F character movement and Ctrl-Left/Right word movement across drafts, searches, agent/session names, project forms, resource paths, and runtime directories. Unicode alphanumeric/underscore word boundaries cross punctuation, whitespace, and newlines while preserving UTF-8 and making boundary movements complete no-ops. Caret-only routing preserves text, validation/recovery state, dirty state, search results, and pending autosave; configuration and append-only interactions remain excluded. Updated contextual editor help and added terminal normalization, all-editor state-preservation, Unicode boundary, and inert-input regressions. Formatting, strict workspace Clippy, and all 1124 Rust workspace tests passed.
