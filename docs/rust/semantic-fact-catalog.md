...
| FCT-016 | AsynchronousMessageSent | canonical | installation-private, peer-addressed, or account-addressed; sender installation root | Address/scope authority and chosen context parents; an asynchronous continuation cites its exact root | local-installation, mailbox-grant, or account-membership role by scope | Root addressing and typed text semantics match scope; an explicit thread retains an asynchronous root’s scope, project and correlation; private non-project exchanges preserve the participant pair in either direction, while project inputs retain the original sender and recipient | Missing authority/context/root defers; addressed incomplete observation allowed | Distinct roots accumulate; stable public message-ID collision conflicts; mismatched continuation fails closed | New asynchronous conversation/input or exact continuation of one exchange | canonical-permanent | thread/message IDs, purpose, delivery audience, incomplete flag |
...
| FCT-022 | HarnessActivityRecorded | canonical | installation-private or account-addressed; source agent installation root | Scope authority and prior logical-key frontier when superseding a snapshot; attributed activity directly cites its exact project dispatch | local-installation or account-membership role | Source is full agent mailbox; provider/session/operation/kind/item/runtime/positive sequence and bounded content are typed; optional project provenance must match the dispatch's project, captured assignment/provider session, thread, and agent mailbox | Missing authority/key frontier or dispatch defers; no incomplete actionable projection | Snapshots/progress use sequenced-snapshot rule; completed item at same semantic identity with unequal content conflicts; terminal items accumulate; changed attribution conflicts | Non-actionable conversation activity; reducer-selected winners only in compacted view | canonical-compacted-view | logical key, source sequence, optional project/dispatch/assignment/thread, winner/conflict, status/truncation, presentation position |
...
| FCT-048 | RemoteProjectCommandOutcome | remote-control | account-addressed; target project-home root | Exact request and receipt; committed canonical project head/facts when outcome is committed | project-home role | Terminal outcome is committed or rejected with typed definite/unknown runtime result and optional signed external-worktree warning; committed head exists and descends expected history | Missing request/receipt/committed fact defers | Unequal terminal outcomes for one command conflict and no terminal winner is reported | Advances remote-command view to committed/rejected plus runtime result and any retained-external-state warning; canonical project facts remain sole state authority | control-permanent | command ID, terminal stage, current head, typed result/error/uncertainty/warning |
| FCT-049 | ConversationArchived | canonical | installation-private for direct/personal conversations; account-addressed for project conversations | Exact conversation identity and at least one causally prior fact in that conversation | local-installation or active account membership | Archive identity matches a causally prior conversation in the same scope | Missing conversation history defers; mismatched identity is invalid | Multiple valid archive facts are equivalent absorbing evidence | Marks the entire immutable transcript archived without hiding or mutating its project | canonical-permanent | conversation identity and complete archive fact evidence |
