...
| IDN-007 | redesign | required | Installation-local configuration | Provider, theme, and Codex defaults are typed unsigned daemon-owned configuration; field updates persist before live publication, while relay policy stays in its typed relay API. | `docs/rust/identity-persistence.md`; identity/config CLI tests |
...
| MSG-007 | redesign | required | Ordered technical disclosure | Namespaced technical sections and exact activity detail are full-pane typed child routes. They are display diagnostics, never authority, routing, or ordinary-summary input, and Escape returns to the exact parent identity. | `docs/rust/tui.md`; typed TUI routing/semantics/evidence/activity mapping and render tests |
...
| MSG-026 | redesign | required | Continuous conversation viewport | Conversation rendering paints intersecting entry slices from a stable entry-plus-row position; row and entry navigation can reach every region of oversized content, clipped edges carry continuation cues, and explicit follow-tail stays at the newest row. | `docs/rust/tui.md`; pure viewport/render/cache tests and installed PTY regression |
...
| NET-010 | redesign | required | Relay configuration and status | Typed durable relay policy controls read/write/auth behavior and wakes the live engine; status exposes actionable health without secrets. | README; application/relay/CLI/TUI packages |
...
| RUN-010 | retain | required | Durable automatic work reconciliation | Startup, direct invalidations, explicit recovery, and bounded immediate continuations wake runnable offline workers without relying on client timing or a recurring repair scan. | Completed Go finding; harness/node packages |
...
| CLI-010 | redesign | required | Administrative workflows | Identity, human, peer, capability, relay, sync, status, repair, node, agent/session, and configuration operations have client commands. Identity stays offline; configuration uses its running daemon or exclusive offline fallback; remaining administration uses the local API. | `docs/rust/cli.md`; administration parser/help/machine-output tests; named-agent, relay, repair, runtime, and project foreground E2E tests |
...
| CLI-013 | redesign | required | Pure Ratatui state/effect and typed navigation architecture | `UiModel + UiEvent` produces effects; one encapsulated typed path owns canonical navigation; each active route renders the complete content pane; renderer performs no I/O or mutation; resizing changes geometry only; materialized Inbox views retain identity-scoped coherence and stale suppression. | `docs/rust/tui.md`; `hq-tui` route algebra, model, render, resize, materialized navigation, stale/cache/reconnect, and no-loading tests; `hq-node` retained-startup, command/observation ownership, blocked-command selection, shutdown, mapping, and effect-identity tests; architecture verifier |
...
| CLI-015 | retain | required | TUI durable editing and workflow state | Drafts, target reselection, typed workflow routes, stable entry-plus-row viewport positions, and follow-tail intent survive reload/reconnect/resize as appropriate. A newly terminal visible agent turn opens its exact follow-up draft without replacing existing input; stale effects cannot steal navigation. | `docs/rust/tui.md`; store restart/conflict/stale-target contracts; pure autosave/save-before-close/save-before-submit/conflict/reload/reconnect/resize/terminal-follow-up and route-correlation tests; responsive full-pane render tests |
...
| CLI-017 | redesign | required | TUI project-first flows | Create/select/reopen/assign/activate/handoff/close/archive/resource/worktree flows use full-pane typed routes, expose conflicts and durable operation state, install canonical cross-workspace destinations, and replay the exact nonterminal worktree command until the exact created project is authoritative. | `docs/projects.md`; `docs/rust/projects-workspace.md`; pure TUI navigation/coordinator/render tests; local-API retained-frame tests; TUI executor continuation tests |
...
| `hq config get/set` | Manage typed installation-local provider, theme, and future-launch preferences. | redesign: `IDN-007`, `CLI-010` |
...
| Mixed message/activity history and canonical scrolling | Consume reducer order, expose activity separately, and preserve stable entry-plus-row viewport positions. | retain: `MSG-008` through `MSG-011`, `CLI-015`; redesign: `MSG-026` |
...
| Project-first creation, resource, activation, handoff, and lifecycle routes | Expose all project invariants, conflicts, full-pane confirmations, and saga outcomes; compose message entry through canonical Inbox routes. | redesign: `PRJ-001` through `PRJ-020`, `CLI-017` |
| Typed paths, Back, deep links, resize, keyboard/mouse interaction, Markdown, and responsive layout | Keep one canonical hierarchy at every width without preserving Go keys, percentages, colors, cells, pane focus, or incidental return history. | redesign/drop boundary: `CLI-013`, `CLI-018`, `CLI-022` |
