...
| CMP-015 | redesign | required | Codex provider version compatibility | Select and pin a current Rust-era baseline from official schema and installed evidence. | ADR 0003; Codex package |
...
| MSG-006 | redesign | required | Typed presentation and correlation | Kind, provider/session/operation/item/request identity, and action grouping are typed fields, never body parsing. | `docs/design.md`; domain/conversation packages |
| MSG-007 | redesign | required | Ordered technical disclosure | Bounded namespaced technical sections are display diagnostics and never authority or routing input. | README and `docs/design.md`; domain/TUI packages |
...
| MSG-009 | redesign | required | Unified conversation entries | Queries expose a typed message/activity union in canonical order with stable event identity. | `docs/design.md`; reducer/store/application packages |
| MSG-010 | retain | required | Activity is non-actionable | Activity cannot create inbox/unread work or become a reply, archive, draft, delivery, or final-answer target. | `docs/harnesses.md`; activity reducer/TUI packages |
| MSG-011 | redesign | required | Activity coalescing and retention | Stable canonical keys select snapshots/progress winners while terminal/completed items remain history. | `docs/events.md`; activity reducer/store packages |
| MSG-012 | retain | required | Output before activity | A normalized provider item persists canonical output before its related activity and reconciles partial success. | `docs/harnesses.md`; harness/store packages |
...
| ID | Classification | Release | Capability or behavior | Rust contract | Evidence and downstream owner |
| --- | --- | --- | --- | --- | --- |
| RUN-001 | redesign | required | Durable named agents | Lowercase installation-local names bind/adopt mailboxes, remain reserved after retirement, and derive from facts. | README; agent reducer/application packages |
| RUN-002 | retain | required | Durable session versus runtime presence | Selection/history survive restarts; processes, leases, phases, and caller environment remain local operational state. | `docs/harnesses.md`; agent reducer/harness packages |
...
| RUN-006 | redesign | required | Codex adapter | Privately implement the selected baseline's handshake, session, turn, request, event, transport, and process behavior. | ADR 0003; Codex package |
| RUN-007 | retain | required | Exact session readiness | Select only after acknowledged start/resume with a matching nonempty durable ID; never silently replace a missing resume. | `docs/harnesses.md`; harness/Codex packages |
...
| RUN-009 | retain | required | One logical worker owner | Leases/tokens prevent duplicate named-agent workers while allowing crash expiry and exact-owner revival. | README; harness supervisor package |
...
| RUN-011 | retain | required | Bounded FIFO plus keyed coalescing | Durable events backpressure; replaceable snapshots coalesce at the tail without reordering intervening work. | `docs/harnesses.md`; harness package |
| RUN-012 | retain | required | Structured interactive requests | Non-secret questions, approvals, permission scopes, and supported MCP forms/URLs receive exactly one validated response. | README and harness docs; harness/Codex packages |
...
| RUN-014 | retain | required | Environment copy/redaction | Caller environment is copied only at the control boundary, retained only in documented memory, wiped, and never serialized/logged by HQ. | `docs/design.md`; harness/node security tests |
| RUN-015 | retain | required | Graceful provider drain and kill | Stop intake, cancel waits, drain accepted work, checkpoint uncertainty, close, escalate kill, wait, and release ownership. | `docs/harnesses.md`; harness/node packages |
| RUN-016 | redesign | required | Typed provider failure causes | EOF, malformed/oversized protocol, child exit, write/read failure, unsupported request, and drain failure remain distinguishable. | `docs/harnesses.md`; Codex/application packages |
| RUN-017 | redesign | required | Session history and naming | Preserve scoped bindings, selection, repository context, launch directory, mutable display names, retirement, and namespace isolation. | README; agent reducer/store packages |
...
| RUN-021 | retain | required | Additive notification tolerance and blocking-request fail-closed policy | Ignore unknown additive notifications, reject unsupported authority-bearing server requests, and terminate with a typed compatibility cause. | `docs/harnesses.md`; Codex conformance tests |
...
| PRJ-004 | redesign | required | Desired resources versus active claims | Durable desired membership, active claim epochs, health observations, and release assessments are distinct. | `docs/projects.md`; project reducer/store |
...
| PRJ-007 | redesign | required | Reversible lifecycle | Open/closed plus operational preparing/closing and constrained archive state follow explicit transitions. | `docs/projects.md`; project reducer/application |
...
| PRJ-009 | retain | required | Close/archive do not touch resources | Releasing advisory claims never deletes/modifies files, worktrees, branches, or containers. | `docs/projects.md`; project workflow/security |
| PRJ-010 | redesign | required | Path identity and conflict | Home-qualified absolute canonical locators detect equal/ancestor/descendant conflicts while preserving human spelling. | `docs/projects.md`; path-resource package |
| PRJ-011 | retain | required | Missing-path and symlink revalidation | Reserve through nearest existing ancestor and fail/warn on later identity disagreement instead of silently changing it. | `docs/projects.md`; path-resource package |
| PRJ-012 | redesign | required | Resource health and release assessment | Typed health plus clean/dirty/unknown/not-applicable observations inform workflows without mutating lifecycle automatically. | `docs/projects.md`; resource/workflow packages |
| PRJ-013 | redesign | required | Primary path and launch override | Defaults are explicit; outside-claim overrides warn, and invalid resumed directories require a human decision. | `docs/projects.md`; project application/harness |
...
| PRJ-015 | redesign | required | Handoff and forced takeover | Graceful handoff requires quiescence; explicit force revokes HQ authority while disclosing possible external access. | `docs/projects.md`; project workflow package |
...
| PRJ-017 | retain | required | Late output classification | Preserve attributable history from inactive assignments without granting current authority or masquerading as current output. | `docs/projects.md`; project reducer/harness |
| PRJ-018 | redesign | required | Remote project control | Active human devices queue authenticated commands/results to the home and expose accepted/received/committed/rejected/runtime stages. | `docs/projects.md`; remote-control/application packages |
| PRJ-019 | redesign | required | Recoverable Git worktree provisioning | Reserve destination, reconcile Git creation, create project, and compensate/retry with one stable operation ID. | `docs/projects.md`; project workflow/Git adapter |
...
| ID | Classification | Release | Capability or behavior | Rust contract | Evidence and downstream owner |
| --- | --- | --- | --- | --- | --- |
| CLI-001 | redesign | required | Reconnecting local API | Strict framed v1 negotiation, build metadata, typed requests/results/errors, reconnect, and full-snapshot recovery. | Rewrite design; local API package |
...
| CLI-003 | retain | required | Nonblocking coalesced invalidations | Revision/topic/full-snapshot wakes have bounded one-pending behavior and never block commit. | `docs/design.md`; local API/node packages |
| CLI-004 | redesign | required | Node auto-start and readiness | Concurrent clients converge on one owner and receive phase/path/cause/action diagnostics on failure. | README and rust port; node/local client packages |
| CLI-005 | redesign | required | Lifecycle status, stop, and restart | Typed lifecycle control drains ownership; clients reconnect/resubscribe and detect build/version incompatibility. | README; node/local API packages |
...
| CLI-012 | redesign | required | Machine-readable automation | Supported commands emit stable typed Rust-era data and errors without preserving Go JSON. | README; local API/CLI packages |
...
| CLI-015 | retain | required | TUI durable editing state | Drafts, target reselection, focus, modal state, and logical scroll anchors survive reload/reconnect/resize as appropriate. | README and TUI work; TUI package |
| CLI-016 | redesign | required | TUI agent/session management | Search agents, inspect presence/history, start/resume/rename/stop, and confirm live switches without losing mailbox state. | README; TUI/application packages |
...
| CLI-019 | redesign | required | Embedded/installed agent guidance | Concise help explains messaging, retries, sync, causal incompleteness, and human-owned administration. | `internal/agenthelp`; CLI/packaging packages |
| CLI-020 | redesign | required | One installed `hq` executable | Client, TUI, lifecycle, and explicit foreground node roles share distribution while keeping internal boundaries. | ADR 0001; node/CLI/packaging packages |
...
| CLI-024 | redesign | required | Help and version inspection | Ship product, workflow, protocol/build, and compatibility guidance without preserving Go help prose or version output. | README and `cmd/hq/main.go`; CLI/packaging packages |
