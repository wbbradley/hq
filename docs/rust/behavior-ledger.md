...
| RUN-006 | redesign | required | Codex adapter | Privately implement the selected baseline's handshake, session, turn, request, event, transport, and process behavior. | ADR 0003; `docs/codex-adapter-v1.md`; `hq-codex` characterization and conformance tests |
...
| RUN-012 | retain | required | Structured interactive requests | Non-secret questions, approvals, permission scopes, and supported MCP forms/URLs receive exactly one validated response. | `docs/codex-adapter-v1.md`; `hq-codex` closed-shape request tests |
...
| RUN-016 | redesign | required | Typed provider failure causes | EOF, malformed/oversized protocol, child exit, write/read failure, unsupported request, and drain failure remain distinguishable. | `docs/codex-adapter-v1.md`; neutral harness error classes; `hq-codex` transport/process tests |
