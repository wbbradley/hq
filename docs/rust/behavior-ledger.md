...
| CLI-015 | retain | required | TUI durable editing state | Drafts, target reselection, focus, modal state, and logical scroll anchors survive reload/reconnect/resize as appropriate. A newly terminal visible agent turn opens its exact follow-up draft without replacing an existing draft. | `docs/rust/tui.md`; store restart/conflict/stale-target contracts; pure autosave/save-before-close/save-before-submit/conflict/reload/reconnect/resize/terminal-follow-up tests; responsive modal render tests |
