...
| CLI-016 | redesign | required | TUI agent/session management | Search agents, inspect presence/history, start/resume/rename/stop, and confirm live switches without losing mailbox state. | `docs/rust/tui.md`; pure stable-search/detail/create/rename/retirement model and responsive render tests; exact executor/client-command mapping; installed TUI/CLI agent-create/restart parity; managed-session lifecycle remains queued |
