...
| ID | Classification | Release | Capability or behavior | Rust contract | Evidence and downstream owner |
| --- | --- | --- | --- | --- | --- |
| CLI-001 | redesign | required | Reconnecting local API | Strict framed v1 negotiation, build metadata, typed requests/results/errors, reconnect, and full-snapshot recovery. | `docs/rust/cli.md`; `hq-local-api` reconnecting-client idle-poll and blocking-runner tests; real subscribed invalidation/restart test in `hq-node/tests/unix_node_cli.rs` |
...
| CLI-003 | retain | required | Nonblocking coalesced invalidations | Revision/topic/full-snapshot wakes have bounded one-pending behavior and never block commit. | `docs/design.md`; local API subscription tests; TUI executor invalidation and bounded-queue tests |
