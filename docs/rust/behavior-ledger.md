...
| CLI-011 | redesign | required | Project workflows | Expose every required project/resource/worktree/remote-result operation from ADR 0003. | `docs/projects.md`; project list/show parser, authoritative snapshot, restart E2E, and CLI projection fixtures; remaining mutations queued |
| CLI-012 | redesign | required | Machine-readable automation | Supported commands emit stable typed Rust-era data and errors without preserving Go JSON. | `docs/rust/cli.md`; CLI machine-output, project-catalog determinism, and redaction fixtures |
