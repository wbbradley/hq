...
| CLI-015 | retain | required | TUI durable editing state | Drafts, target reselection, focus, modal state, stable entry-plus-row viewport positions, and follow-tail intent survive reload/reconnect/resize as appropriate. A newly terminal visible agent turn opens its exact follow-up draft without replacing an existing draft. | `docs/rust/tui.md`; store restart/conflict/stale-target contracts; pure autosave/save-before-close/save-before-submit/conflict/reload/reconnect/resize/terminal-follow-up tests; responsive modal render tests |
...
| CLI-017 | redesign | required | TUI project-first flows | Create/select/reopen/assign/activate/handoff/close/archive/resource/worktree flows expose conflicts and durable operation state. Guided creation retains typed Back destinations and replays the exact nonterminal worktree command until the exact created project is authoritative. | `docs/projects.md`; pure TUI coordinator/render tests; local-API retained-frame tests; TUI executor continuation tests |
