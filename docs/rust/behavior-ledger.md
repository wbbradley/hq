...
| CLI-003 | retain | required | Nonblocking coalesced invalidations | Store revision and per-subscriber revision/topic/full-snapshot wakes have bounded latest-value behavior, wake the daemon and an independent TUI observation owner directly, and never block commit or wait for command completion. | `docs/design.md`; store actor wake tests; local session-pump pressure test; local API subscription tests; Unix read-interrupt test; blocked-command and bounded-queue TUI executor tests |
