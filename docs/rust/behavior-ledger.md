...
| CLI-003 | retain | required | Nonblocking coalesced invalidations | Store revision and per-subscriber revision/topic/full-snapshot wakes have bounded latest-value behavior, wake the daemon directly, and never block commit. | `docs/design.md`; store actor wake tests; local session-pump pressure test; local API subscription tests; TUI executor invalidation and bounded-queue tests |
