...
| RUN-007 | retain | required | Exact session readiness | Select only after acknowledged start/resume with a matching nonempty durable ID; never silently replace a missing resume. | `docs/harnesses.md`; application binding/context/selection planner tests; supervisor recovery tests |
...
| RUN-014 | retain | required | Environment copy/redaction | Caller environment is copied only at the control boundary, retained only in documented memory, wiped, and never serialized/logged by HQ. | `docs/design.md`; application launch-environment tests; local API protocol/reconnect tests; harness security tests |
