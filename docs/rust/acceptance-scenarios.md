...
| ACT-014 | provider-approval-responder | Fake Codex blocks one project turn on command approval while another conversation and modeless views remain usable | Installed TUI / reconnect / server-session race | Approval needed appears without interrupting composition in its exactly mapped conversation; explicit Tab handoff replaces the editor, and resolution restores its draft and caret; its transcript and tail state survive; view switching and unrelated replies remain available; one request-correlated choice reaches Codex, output resumes, equal answer replay reconciles, and disconnect fail-closes stale requests | Conversation-scoped interactive authority |
| ACT-015 | persistent-conversation-composition | A project or direct conversation is opened, including while agents are running | Typed conversation context from authoritative observation | The conversation-scoped composer is available immediately; agent completion and message inspection never retarget or replace the retained draft | Conversation composition |
