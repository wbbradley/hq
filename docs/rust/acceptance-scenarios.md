...
| ACT-014 | provider-approval-responder | Fake Codex blocks one project turn on command approval while another conversation and modeless views remain usable | Installed TUI / reconnect / server-session race | Approval appears inline only in its exactly mapped conversation; its transcript, viewport, and draft survive; view switching and unrelated replies remain available; one request-correlated choice reaches Codex, output resumes, equal answer replay reconciles, and disconnect fail-closes stale requests | Conversation-scoped interactive authority |
| ACT-015 | terminal-turn-follow-up | Selected project or direct conversation changes from one running agent turn to a new terminal turn | Materialized observation and fallback page load | Exact continuation/reply draft opens; initial completed history, another running turn, or an existing draft prevents an unsolicited replacement | Follow-up composition |
| ACT-016 | orphaned-running-turn | Provider EOF or poll failure follows a durable running agent turn, or the daemon dies before teardown and later resumes the exact session | Supervisor teardown and restart recovery | Before releasing or reusing exact worker ownership, HQ durably advances the running turn to interrupted; retries are idempotent, while terminal operations and other sessions remain unchanged | Worker lifecycle reconciliation |
...
| PRJ-027 | mailbox-receipt-before-runtime | Submit a project draft while reconciliation or provider acceptance is blocked | Pure model / component / installed TUI | The `You` sender rule and message with `Pending` paint with the command effect; the durable mailbox receipt returns before sequencing/runtime delivery, and background recovery converges once without duplicate text | Submission/delivery separation |
